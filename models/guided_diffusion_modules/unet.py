...
from models.guided_diffusion_modules.rgcspelan import RGCSPELAN
from models.guided_diffusion_modules.lawds import LAWDS
from models.guided_diffusion_modules.DSUB import DSUB
from models.guided_diffusion_modules.CASAB import CASAB
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_conv:
            x = self.conv(self.DSUB(x))
            print("DSUB在upsample应用成功")
        return x
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, out_channel=None):
        ...
        if use_conv:
            ...
            # )
            self.op = LAWDS(self.channels, self.out_channel, group=16)
            print("LAWDS在downsample应用成功")
        else:
...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            ...
            RGCSPELAN(self.out_channel, self.out_channel, n=2, scale=0.5, e=0.5),
            print("RGCSPELAN在in_layers应用成功")
        )
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.middle_block = EmbedSequential(
            ...
            CASAB(ch),
            print("CASAB在self.middle_block中应用成功"),
        )
        self._feature_size += ch
